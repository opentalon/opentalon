...
- **Memories** are scoped as **general** (shared; `actor_id` NULL) or **per-actor** (one row per actor). The LLM receives general context (config rules, general stored rules, tools) plus the current actor’s stored rules. General memory is the place for **admin-created shared rules**; the write path for general memory (config bootstrap, permission plugin, or admin API) can be restricted in a future release.
